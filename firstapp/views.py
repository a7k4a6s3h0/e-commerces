...
def validate(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            res = int("".join(s))
            try:
                curr_otp = OTP.objects.get(OTP_digit=res)
                if curr_otp.expiry_time < timezone.now():
                    messages.info(request,'Your OTP has been expired! Please generate a new one')
                    return redirect('/signup')
            ...
            print(res, curr_otp.OTP_digit)
            if res == curr_otp.OTP_digit:
                curr_otp.is_verified = True
                curr_otp.save()

                return render(request, 'site/signup3.html', {'data': literal_eval(request.COOKIES.get('details')) })
            else:
                messages.error(request,'Invalid OTP')
                return redirect('validate')
...
def resent_otp(request):
    ...
    try:
        user_info = literal_eval(request.COOKIES.get('details'))
    except Exception as e:
    ...
    try:
            ...
            otp_instance = OTP.objects.get(author=user_info['username'])

            if otp_instance.request_times < 0:
                messages.error(request, "You have exceeded the limit for resending OTP")
...
def user_details(request):
    if request.method =="POST":
        ...
        if info['username'] == username1 and info['password'] == password2 and info['email'] == email :
            if referal:
...
def apply_coupen(request):
    ...
    if coupens.objects.filter(coup_name  = code1).exists():
        ...
        if not coupen.is_experied:
            return JsonResponse({"result": "Coupon is expired!"})
