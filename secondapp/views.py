...
def change_coupen_status(request):
    ...
    if status and id :
        try:
            coupon_change = coupens.objects.filter(id=id).update(is_experied=status)
            if coupon_change:
                return JsonResponse({
        ...
        except coupens.DoesNotExist as e:
            return JsonResponse({
                "message": str(e),
            })
    else:
    ...
    if from_date and to_date is not None:
        if choose == 'pdf':
            ...
            if order_details:
                ...
                messages.error(request, 'no order left')
